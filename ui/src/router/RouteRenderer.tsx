...
const { useContext, useEffect, Suspense, useState, useTransition } = React;
...
export default function RouterRenderer() {
  ...
  useEffect(() => {
    ...
    console.log('currentEntry',currentEntry?.location?.pathname)
    console.log('routeEntry',routeEntry?.location?.pathname)
    console.log(currentEntry !== routeEntry)
    if (currentEntry !== routeEntry) {
      // if there was a concurrent modification, rerender and exit
      console.log('setting entry concurrently')
